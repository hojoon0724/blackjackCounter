...
export default function DeckAmountInput({
  ...
  return (
    <div className="deck-amount-input-container flex-row align-center">
      ...
      <div
        className="deck-amount-minus-button"
        onClick={() => setDeckAmount(prev => Math.max(1, prev - 1))}
      >
        -
      </div>
      <input
      ...
      />
      <div
        className="deck-amount-plus-button"
        onClick={() => setDeckAmount(prev => prev + 1)}
      >
        +
      </div>
      <button className="action-button shuffle-button">Shuffle</button>
    </div>
  );
}
